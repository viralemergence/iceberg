...
# Cleaning October Climate Data ####

library(tidyverse); library(fs)

setwd(here::here())

Files <- "Iceberg Files/CHELSA/Input Rasters" %>% list.files(full.names = T)# %>% substr(1,2)
...
dir_create("Iceberg Files/CHELSA/Intermediate Rasters")
...
for(i in 1:length(Files)){
  ...
  unzip(Files[i], 
        exdir = paste0("Iceberg Files/CHELSA/Intermediate Rasters/", 
                       FilesDF[i, "Name"]))

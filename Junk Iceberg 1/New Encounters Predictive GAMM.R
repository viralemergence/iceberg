...
# Predictive new encounters model ####
...
NEPredictModels <- TestDFList <- list()
...
TestDFList[[Pipeline]] <- NEPredictModels[[Pipeline]] <- list()

TestDFList[[Pipeline]][[PredReps[x]]] <- BlankDF %>%
  mutate(OverlapSum = NewIntersectsManual[[Pipeline]][,paste0("Overlap.",PredReps[x],Pipeline)])

LandUse <- brick(paste0("Iceberg Input Files/","LandUse", PredReps[x], ".grd"))

# Non-PP
TestDFList[[Pipeline]][[PredReps[x]]][,names(LandUse)] <- 
  (LandUse %>% names %>% lapply(function(a) values(LandUse[[a]])) %>% bind_cols)[-Sea,]

TestDFList[[Pipeline]][[PredReps[x]]] <- TestDFList[[Pipeline]][[PredReps[x]]] %>% na.omit

Covar <- c(paste0("s(",c("Richness", 
                         "Elevation",
                         "Precipitation", 
                         "Temperature",
                         "EquatorDistance"),")")[1:2], names(LandUse))

Formula <- Covar %>% paste(collapse = " + ") %>% paste0("OverlapSum ~ ", .) %>% as.formula()

NEPredictModels[[Pipeline]][[PredReps[x]]] <- list()

NEPredictModels[[Pipeline]][[PredReps[x]]]$OverlapSum <- bam(Formula,
                                                             data = TestDFList[[Pipeline]][[PredReps[x]]], 
                                                             family = nb()
)
...
Model <- NEPredictModels[[Pipeline]][[PredReps[x]]]$OverlapSum
...
TemperatureRange <- seq(from = min(TestDFList[[Pipeline]][[PredReps[x]]]$Temperature),
                        to = max(TestDFList[[Pipeline]][[PredReps[x]]]$Temperature),
                        length = 21)  %>% 
  c(mean(TestDFList[[Pipeline]][[PredReps[x]]]$Temperature))

PrecipitationRange <- seq(from = min(TestDFList[[Pipeline]][[PredReps[x]]]$Precipitation),
                          to = max(TestDFList[[Pipeline]][[PredReps[x]]]$Precipitation),
                          length = 21)  %>% 
  c(mean(TestDFList[[Pipeline]][[PredReps[x]]]$Precipitation))

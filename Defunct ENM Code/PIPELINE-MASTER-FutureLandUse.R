...
future.dir <- 'D:/ICEBERG/RawENMs/PPM/OtherEnvPred/he4570/BinaryMaps'
outdir <- 'D:/ICEBERG/RawENMs/PPM/OtherEnvPred/he4570 PROCESSED'
...
for (i in 1:length(raw.names)) {
  ...
  if(spnamei %in% drop.manual) {
    ...
    writeRaster(input,paste(paste(outdir,spnamei,sep='/'),'.tif',sep=''),
                overwrite=TRUE)

# "2_Iceberg Data Import.R" ####

# FullRangeAdj <- IcebergAdjList$Currents
FullRangeAdj <- CurrentsRangeAdjA

rownames(IcebergAdj) <- colnames(IcebergAdj)  <- colnames(IcebergAdj) %>% str_remove(".tif")
...
# Phylogenetic Data Import

if(!file.exists("Iceberg Input Files/HP3-ST_PDmatrix.csv")){
...
FinalHostMatrix %>% slice(sample(1:n(), 100000)) %>% ggplot(aes(SpaceA, SpaceB)) + geom_point()
